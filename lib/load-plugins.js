...
module.exports = function (serviceLocator, eventEmitter, plugins, dependencies, cb) {
  var loadedPlugins = []
  ...
  function loadAsync(plugin, cb) {
    var args = getDependencyInstances(serviceLocator, plugin).concat(function (error, instance) {
      ...
      eventEmitter.emit('afterLoad', plugin, instance)
      eventEmitter.emit('afterLoad:' + plugin.name, plugin, instance)
    ...
    eventEmitter.emit('beforeLoad', plugin)
    eventEmitter.emit('beforeLoad:' + plugin.name, plugin)
